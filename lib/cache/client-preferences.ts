...
function writeSessionValue(key: string, value: unknown) {
    ...
    try {
        ...
        // Embedded browsers may disable storage. Preferences are optional.
    }
}
...
export function readBookPreferences(): BookPreferences | null {
    ...
    const validClassTypes = new Set(["all", "regular", "special"]);
    const validIntensities = new Set(["all", "A", "B", "I"]);
    if (
        !value ||
        !isReasonableDate(value.visibleMonth, 3) ||
        typeof value.instructorId !== "string" ||
        value.instructorId.length > 128 ||
        !validClassTypes.has(value.classType) ||
        !validIntensities.has(value.intensity) ||
