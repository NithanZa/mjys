...
- [x] Booking or cancellation refreshes bookings and the member's package state.
- [x] No bookings, member data, or auth tokens are persisted to device storage.

## Implementation

- `BookingsProvider` owns booking state and API mutations.
- It is mounted once in `app/(shell)/layout.tsx`, below member and purchase providers.
- Overlapping requests are sequenced so stale responses cannot replace current state.
