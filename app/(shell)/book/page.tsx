...
import { fetchClasses, fetchOccurrence, OccurrenceView } from "@/lib/api/classes";
...
export default function BookPage() {
  ...
  useEffect(() => {
    ...
    if (preferences) {
      ...
      const restoredMonth =
        storedMonth >= navigationRange.from &&
        storedMonth <= navigationRange.to
          ? storedMonth
          : today;
  ...
  const visibleScheduleLoading =
    loading || loadingMonthKeys.has(classMonthKey(visibleMonth));
  const feedLoading =
    loading ||
    ...
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full pb-12 items-start mt-4">
        ...
        <div className="lg:col-span-7 flex flex-col gap-4">
          ...
          <InlineCalendar
            key={calendarInitialMonth.toISOString()}
            selected={selected}
